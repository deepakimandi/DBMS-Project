$(document).ready(function(){
	...
	// brand();
	product();
	//sup() is a funtion fetching supplier record from database whenever page is load
	function sup(){
		$.ajax({
			url	:	"action.php",
			method:	"POST",
	...
	/*	when page is load successfully then there is a list of suppliers when user click on supplier we will get supplier id and
		according to id we will show products
	*/
	$("body").delegate(".supplier","click",function(event){
		...
		var sid = $(this).attr('sid');
			...
			url		:	"action.php",
			method	:	"POST",
			data	:	{get_seleted_Supplier:1,supplier_id:sid},
			success	:	function(data){
				$("#get_product").html(data);
				if($("body").width() < 480){
					$("body").scrollTop(683);
	...
	*/
	$("body").delegate(".qty","keyup",function(event){
		...
		$('.net_total').html("Total Payable Amount : Rs. " +net_total);
	...
	*/
	...
	function paymentMethods2(){
		$('.overlay').show();
		$.ajax({
			url : "action.php",
			method : "POST",
			data : {paymentMethods2:1},
			success : function(data){
				$('.overlay').hide();
				$("#payment_2").html(data);
			}
